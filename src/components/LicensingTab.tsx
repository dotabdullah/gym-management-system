        ...
        <div className="lg:col-span-2 bg-[#161B22] border border-slate-800 rounded-3xl p-5 space-y-6 shadow-sm" id="activation-card">
          <div className="flex justify-between items-start" id="activation-header">
            ...
            {license.status === 'activated' ? (
              <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold px-3 py-1 rounded-full uppercase flex items-center gap-1.5 animate-pulse">
                <ShieldCheck className="w-4 h-4" /> Locked & Active
              </span>
            ) : (
              <span className="bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold px-3 py-1 rounded-full uppercase">
                Trial Mode
              </span>
            )}
          </div>
        ...
        {isCreatorPanelOpen && (
          <div className="mt-5 pt-5 border-t border-slate-850 space-y-4 animate-scaleUp" id="creator-body">
            <p className="text-xs text-slate-300 leading-relaxed max-w-2xl">
              As the SaaS founder, you will generate subscription codes for gym owners. In a real desktop environment, these keys are generated by your server and sent to your database. Use this simulator to generate a valid key, copy it, and try activating the software node above!
            </p>

            <div className="flex flex-wrap items-center gap-3" id="generator-actions">
              <button
                onClick={handleGenerateKey}
                className="bg-slate-900 border border-slate-800 hover:border-lime-400 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                id="generate-key-btn"
              >
                <RefreshCw className="w-3.5 h-3.5" /> Generate Active Key
              </button>

              {generatedKey && (
                <div className="flex items-center bg-slate-900 border border-slate-800 rounded-xl pl-3.5 pr-2 py-1.5" id="generated-key-box">
                  <span className="text-yellow-400 font-mono text-xs font-bold tracking-wider mr-4">{generatedKey}</span>
                  <button
                    onClick={handleCopy}
                    className="p-1.5 rounded-lg bg-slate-800 hover:bg-lime-400 hover:text-black text-white text-xs font-bold transition-all cursor-pointer"
                    id="copy-key-btn"
                  >
          ...
          <div className="bg-[#161B22] border border-slate-800 rounded-3xl max-w-sm w-full shadow-2xl p-6" id="passcode-modal-content">
            ...
            <form onSubmit={handleVerifyPasscode} className="mt-4 space-y-4" id="passcode-form">
              <div>
                ...
                <span className="text-slate-500 text-[10px] mt-1.5 block leading-normal">
                  Hint: As the developer, your default creator key is <code className="text-lime-400 font-mono">ironcreator77</code>. You can change this passcode once inside.
                </span>
              </div>
